<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>

<!-- Left side column. contains the logo and sidebar -->
<aside class="main-sidebar">
    <!-- sidebar: style can be found in sidebar.less -->
    <section class="sidebar">
        <!-- Sidebar user panel -->
        <div class="user-panel">
            <div class="pull-left image">
                <img src="${sessionScope.user.userAvatar}" class="img-circle" alt="User Image">
            </div>
            <div class="pull-left info">
                <p>${sessionScope.user.userDisplayName} (${sessionScope.user.roleName})</p>
                <a href="#"><i class="fa fa-circle text-success"></i>online</a>
            </div>
        </div>
        <!-- sidebar menu: : style can be found in sidebar.less -->
        <ul class="sidebar-menu" data-widget="tree">
            <li class="header">Basic functions</li>
            <li class="treeview">
                <a href="#">
                    <i class="fa fa-user"></i> <span>Basic information</span>
                    <span class="pull-right-container"><i class="fa fa-angle-left pull-right"></i></span>
                </a>
                <ul class="treeview-menu">
                    <li><a href="/admin/profile"><i class="fa fa-circle-o"></i>Edit personal information</a></li>
                    <li><a href="/admin/password"><i class="fa fa-circle-o"></i>Change password</a></li>
                </ul>
            </li>
            <li class="treeview">
                <a href="#">
                    <i class="fa fa-paint-brush"></i> <span>Property management</span>
                    <span class="pull-right-container">
                        <i class="fa fa-angle-left pull-right"></i>
                    </span>
                </a>
                <ul class="treeview-menu">
                    <li><a href="/admin/house"><i class="fa fa-circle-o"></i>Published Property</a></li>
                    <li><a href="/admin/house/publish"><i class="fa fa-circle-o"></i>Publish new Property</a></li>
                </ul>
            </li>
            <li><a href="/admin/order"><i class="fa fa-database"></i> <span>Order management</span></a></li>


            <li class="header">Special functions</li>
            <li><a href="/admin/bill"><i class="fa fa-dollar"></i> <span>Bill management</span></a></li>
            <li><a href="/admin/maintain"><i class="fa fa-gavel"></i> <span>Maintenance management</span></a></li>
            <li class="treeview">
                <a href="#">
                    <i class="fa fa-pencil"></i> <span>Rent management</span>
                    <span class="pull-right-container">
                        <i class="fa fa-angle-left pull-right"></i>
                    </span>
                </a>
                <ul class="treeview-menu">
                    <li><a href="/admin/post"><i class="fa fa-circle-o"></i>Article management</a></li>
                    <li><a href="/admin/comment"><i class="fa fa-circle-o"></i>Message management</a></li>
                </ul>
            </li>
            <li>
                <a href="/admin/feedbackSurvey/dashboard">
                    <i class="fa fa-bar-chart"></i>
                    <span>Feedback Survey Dashboard</span>
                </a>
            </li>

            <li class="header">System management</li>
            <li><a href="/admin/user"><i class="fa fa-cog"></i> <span>User management</span></a></li>
            <li class="treeview">
                <a href="#">
                    <i class="fa fa-bell"></i> <span>Message management</span>
                    <span class="pull-right-container">
                        <i class="fa fa-angle-left pull-right"></i>
                    </span>
                </a>
                <ul class="treeview-menu">
                    <li><a href="/message" target="_blank"><i class="fa fa-circle-o"></i>Message management
                        <c:if test="${sessionScope.user.notReadMessageCount !=0}">
                        <span
                                class="pull-right-container"><small
                                class="label pull-right bg-red">${sessionScope.user.notReadMessageCount}</small></span>
                        </c:if>
                    </a></li>
                    <li><a href="/admin/feedback"><i class="fa fa-circle-o"></i>Feedback management</a></li>
                </ul>
            </li>
            <li><a href="/logout"><i class="fa fa-sign-out"></i> <span>Logout</span></a></li>
        </ul>
    </section>
    <!-- /.sidebar -->
</aside>